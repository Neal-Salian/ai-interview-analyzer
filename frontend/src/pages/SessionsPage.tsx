                        ...
                        <div style={{
                            ...
                            {sessions.map((session) => (
                                ...
                                >
                                    ...
                                    <PanelSection sessionId={selectedSession.session_id} token={token!} />
                                    {/* Action Buttons */}
...
// Paste this component at the bottom of SessionsPage.tsx
function PanelSection({ sessionId, token }: { sessionId: string; token: string }) {
    ...
    return (
        <div style={{ marginTop: '24px', padding: '20px', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)' }}>
                    ...
                    {members.map(m => (
                        <div key={m.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'var(--bg-surface-high)', borderRadius: 'var(--radius)', fontSize: '13px' }}>
                            <div>
                                ...
                                <span style={{ color: 'var(--text-secondary)', marginLeft: '8px' }}>{m.email}</span>
                            </div>
                            <button
            ...
            <button onClick={addMember} disabled={loading} style={{ background: 'var(--accent)', color: '#000', border: 'none', borderRadius: 'var(--radius)', padding: '8px 16px', fontSize: '13px', fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }}>
